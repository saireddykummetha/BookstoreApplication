...
const Cartproduct = ({id,name,price,image,category,total,qty,description}) => {
  ...
  return (
    <div className='bg-slate-200 p-2 sm:p-3 flex flex-col sm:flex-row gap-3 sm:gap-4 rounded border border-slate-300 mb-3'>
      ...
      <h3 className='font-semibold text-slate-600 capitalize text-base sm:text-lg md:text-xl lg:text-xl px-2 sm:px-5 flex-1'>{name}</h3>
      <div className='cursor-pointer py-2 sm:py-4 text-xl sm:text-xl text-slate-700 hover:text-red-700 'onClick={()=>dispatch(deleteCartItem(id))}>
         <MdDelete/>
      </div>
      </div>
    <p className='text-slate-500 font-medium text-sm sm:text-base  sm:px-3  sm:pt-2'>{category}</p>
