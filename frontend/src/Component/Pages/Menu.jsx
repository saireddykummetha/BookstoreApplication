...
const Menu = () => {
  ...
  return (
    <div className='p-2 sm:p-4'>
     ...
     <div className='w-full max-w-3xl mt-4 sm:mt-6 md:mt-10 m-auto  bg-white rounded-lg shadow-lg flex flex-col md:flex-row overflow-hidden'>
      <div className='w-full md:w-1/2 h-64 sm:h-80 md:h-auto flex justify-center items-center bg-gray-200 overflow-hidden'>
        <img src={productDisplay?.image} className='w-full h-120 object-contain hover:scale-105 transition-transform duration-300 p-2 sm:p-4' alt={productDisplay?.name}/>
        
      </div>
      <div className='w-full md:w-1/2 p-4 sm:p-5 md:p-8 flex flex-col justify-between'>
      <div>
      <h3 className='font-semibold text-slate-600 capitalize text-lg sm:text-xl md:text-xl lg:text-xl mb-2'>{productDisplay?.name}</h3>
    ...
    <div className='mb-2 sm:mb-2'>
        <p className='font-semibold text-slate-700  text-sm sm:text-base'>Description:</p>
        <p className='text-slate-500 text-xs sm:text-sm md:text-base'>{productDisplay?.description}</p>
     </div>
     </div>
    <div className='text-center py-3 sm:py-4'>
    <button className='bg-yellow-400 hover:bg-yellow-500 w-full md:w-auto px-3 sm:px-8 cursor-pointer h-9 sm:h-10 rounded-full font-semibold'onClick={handlecartproduct}>Add to Cart</button>    
     </div>
